...
bool InRange( const pnt &I, const Line &L ){
	pnt p= L.F;  pnt q= L.S;  
	double dist_I= P_dist(p,I)+ P_dist(I,q);  double dist_pq= P_dist(p,q);
	if( D_eq( dist_I, dist_pq ) ) return 1;
	return 0;
}

void Line_Line_Intersection( const int  &ind1, const int &ind2 ){
	Line L1= myLines[ ind1 ];  Line L2= myLines[ ind2 ];
	pnt A= L1.F; pnt B= L1.S;  pnt C= L2.F; pnt D= L2.S;

	double m1= (B.Y-A.Y)/(B.X-A.X);  double m2= (D.Y-C.Y)/(D.X-C.X);
	pnt I= pnt( inf, inf );
	if( D_eq( m1, m2 ) ){ //they are parallel
		if( P_eq( L1.F, L2.F ) )  I= L1.F;
		else if( P_eq( L1.F, L2.S ) )  I= L1.F;
		else if( P_eq( L1.S, L2.F ) )  I= L1.S;
		else if( P_eq( L1.S, L2.S ) )  I= L1.S;
		else return;
	}
	else{
		double p= (A-C)^(D-A);  double q= (B-D)^(C-B);
		I= ( p/(p+q) )*B + ( q/(p+q) )*A;
		if( !InRange( I, L1 ) || !InRange( I, L2 ) ) return;
	}
